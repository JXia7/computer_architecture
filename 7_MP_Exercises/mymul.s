...
mymul:  
    ...
    # finish mymul here
    lw $a0,20($sp) # get value of left
    lw $a1,24($sp) # get value of right
    li $t0,1
    
    beq $a0,$t0,Lif # if (left == 1) return_value = right
    b Lelse         # else return_value = (right + mymul(left-1,right))
